    ...
    export const FormHeader = () => {
        ...
        const { event, isInternal, step, totalSteps, teamName, closeForm, members, handlePrev } = useRegistrationContext();
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        {step > 0 && (
                            <div className="flex items-center gap-2 mt-1">
                                <span className={`text-[10px] px-1.5 py-0.5 rounded border font-bold ${isInternal ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-orange-500/10 text-orange-400 border-orange-500/20'}`}>
                                    {isInternal ? "INTERNAL" : "EXTERNAL"}
                                </span>
                                {teamName && <span className="text-yellow-500 font-mono text-sm">/ {teamName}</span>}
                            </div>
                        )}
                    </div>
                    </div>
    ...
    const { isInternal, toggleInternal, handleNext } = useRegistrationContext();

    const handleSelection = (userIsInternal: boolean) => {
        // Only toggle if the current state doesn't match the selection
        if (isInternal !== userIsInternal) {
            toggleInternal();
        }
        
        // TIMING FIX: We use a small timeout to allow the state to update 
        // before moving next, ensuring the context is set correctly.
        setTimeout(() => {
            handleNext();
        }, 50);
    };
    ...
    return (
        <div className="flex flex-col justify-center h-full py-10 space-y-8">
            <div className="text-center space-y-2">
                <h3 className="text-2xl font-black uppercase text-white tracking-wide">
                    Welcome
                </h3>
                <p className="text-zinc-400">
                    Are you currently a student at <span className="text-yellow-500 font-bold">IIEST Shibpur</span>?
                </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                <button
                    ...
                    className="group relative flex flex-col items-center justify-center gap-3 p-6 rounded-2xl border border-zinc-700 bg-zinc-800/50 hover:bg-yellow-500 hover:border-yellow-500 transition-all duration-300"
                >
                    <CheckCircle className="w-10 h-10 text-zinc-500 group-hover:text-black transition-colors" />
                    <span className="text-lg font-bold text-zinc-300 group-hover:text-black">YES</span>
                </button>

                <button
                    onClick={() => handleSelection(false)}
                    className="group relative flex flex-col items-center justify-center gap-3 p-6 rounded-2xl border border-zinc-700 bg-zinc-800/50 hover:bg-white hover:border-white transition-all duration-300"
                >
                    <Users className="w-10 h-10 text-zinc-500 group-hover:text-black transition-colors" />
                    <span className="text-lg font-bold text-zinc-300 group-hover:text-black">NO</span>
                </button>
            </div>
        </div>
    );
